...
async def scrape_data():
    # Only request BANJARMASIN data
    url_template = "https://phinnisi.pelindo.co.id:9021/api/executing/monitoring-operational?page={page}&record=10000&data=&subBranch=MTc="
    ...
    for page_number in range(1, total_pages + 1):
        ...
        # No need to filter by branch since we're only requesting BANJARMASIN data
        filtered_data = [
        ...
        print(f"Found {len(filtered_data)} records on page {page_number}")
        all_records.extend(filtered_data)
        time.sleep(1)

    # Save to CSV only if we have records
    if not all_records:
        print("❌ No records found for BANJARMASIN")
        await browser.close()
        return
    ...
    # Save to CSV
    csv_filename = '/Users/hadipurwana/Library/CloudStorage/GoogleDrive-pjmdataapps@gmail.com/My Drive/WEB/GabungSC/banjarmasin.csv'
    with open(csv_filename, mode='w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        writer.writerow(all_records[0].keys())  # Write header
        for record in all_records:
            writer.writerow(record.values())
    print(f"✅ Data berhasil disimpan dalam file banjarmasin.csv")
    print(f"📊 Total {len(all_records)} records telah disimpan")
